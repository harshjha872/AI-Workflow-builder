  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-8">
      <header className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">AI Workflow Builder</h1>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-sm"
        >
      ...
      {workflows.length === 0 ? (
        <section className="mt-16 flex flex-1 flex-col items-center justify-center text-center">
          <p className="mb-2 text-sm font-medium text-slate-800">No workflows yet.</p>
        ...
        <section className="grid flex-1 grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {workflows.map((wf) => (
            <article
              key={wf.id}
              className="group flex flex-col justify-between rounded-lg border bg-white p-4 shadow-sm hover:border-slate-300"
            >
              <div>
                <h2 className="mb-1 text-sm font-semibold text-slate-900">{wf.name}</h2>
                {wf.description && (
                  <p className="mb-2 text-xs text-slate-500 line-clamp-2">{wf.description}</p>
                )}
